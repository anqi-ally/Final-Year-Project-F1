...
def process_all_scans(base_path="data", show_labels=True):
    ...
    for scan_name in scan_folders:
        ...
        csv_files = [f for f in os.listdir(scan_dir) if f.endswith(".csv")]
        if not csv_files:
            print(f"[跳过] {scan_name}:目录中无 .csv 文件")
            continue

        print(f"[处理中] {scan_name}")
        save_path = os.path.join(scan_dir, f"bmode_{scan_name}.png")  # 保存图像路径（未启用）
        generate_bmode_from_folder(scan_dir, scan_name=scan_name, save_path=save_path)  # 生成并显示图像
