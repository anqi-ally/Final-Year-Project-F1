...
def plot_envelope_matrix(matrix: np.ndarray, title: str = "Envelope Max Value Heatmap", vmin=None, vmax=None) -> None:
    """
    绘制 envelope 最大值的热力图（灰度图），支持统一颜色范围设置。
    ...
    plt.imshow(matrix, cmap='gray', interpolation='nearest', vmin=vmin, vmax=vmax)
    plt.title(title)
    plt.xlabel("Column Index")
    plt.ylabel("Row Index")
    plt.colorbar(label="Envelope Max")
    plt.show()

# 示例调用
matrix = extract_envelope_max_matrix("data/scan_013/filtered1")
plot_envelope_matrix(matrix, vmin=0.0, vmax=1.0)  # 可按实际范围调整
